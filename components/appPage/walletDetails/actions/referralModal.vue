  ...
  >
    ...
    <Tabs v-model="referralTabs" class="referralTabs">
      <TabPane name="m0" class="m0">
        <div class="content">
          ...
          <div class="contentDesc">
            Receive 10 USD worth LINA when a friend adds your code and completes
            10 transactions, up to 1000 quota for phase 1 marketing
          </div>
          <div class="youCodeBox">
            ...
            >
              <div
                ...
                @click="copyYouCode"
              >
                <img src="@/static/copy_hover.svg" />Copy to Clipboard
              </div>
            </Tooltip>
          </div>
      ...
      <TabPane name="m1" class="m1">
        ...
        <div class="title">
          You are adding {{ referralCode.referral_code }}. Are you sure?
        </div>
        <div class="desc">Referral code can only be submitted once.</div>
        <div class="btns">
          <div class="back" @click="referralTabs = 'm0'">No, go back</div>
          <div
            class="confirm"
            :class="{ disabled: disabledSubmitBtn }"
    ...
    >
      ...
      <div class="context">Referral code can only be submitted once.</div>
    </Modal>
  </Modal>
</template>
  ...
  methods: {
    ...
    referralModalChange(status) {
      if (status) {
        this.loading = true;
        api
          .getReferralCode(this.walletAddress)
          .then((res) => {
            if (_.has(res, "data")) {
              this.referralCode = res.data;
              this.addCodeFinish = this.referralCode.referral_code != null;
            }
          })
          .catch((err) => {
    ...
    submitReferralCode() {
      if (!this.disabledSubmitBtn) {
        this.loading = true;
        api
          .addReferralCode(this.walletAddress, this.referralCode.referral_code)
          .then((res) => {
