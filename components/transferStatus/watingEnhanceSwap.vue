...
<template>
  <div id="transferWatingEnhanceSwap">
    ...
    <div class="waitTitle">
      ...
      <template v-if="waitChainChange">
        ...
        >
          <template v-if="isEthereumNetwork(targetNetworkId)"
            >Ethereum</template
          >
          <template v-if="isBinanceNetwork(targetNetworkId)">BSC</template>
      ...
      <template v-else>
        <!-- 已经完成 -->
        <span v-if="currentStep > setupArray.length - 1">Congratulations!</span>
        <!-- 当前有错误 -->
        ...
        <!-- 没完成,没错误,已确认 -->
        <span v-else>
          <template v-if="isMobile"> Interacting </template>
          <template v-else> Interacting with the smart contract </template>
        </span>
      </template>
    </div>
    ...
    <div class="waitDesc">
      ...
      <span v-if="currentStep > setupArray.length - 1">
        ...
        >
        <!-- pc端 -->
        <template v-else>Your transaction has been processed already</template>
      </span>
      <!-- 有错误 -->
      <span class="error" v-else-if="currentErrMsg">{{ currentErrMsg }}</span>
      <!-- 没完成没错误且是移动端 -->
      <span v-else-if="isMobile"> Swap transaction is in progress </span>
    </div>
      ...
      <template v-if="waitChainChange">
        ...
        <template v-if="sourceWalletType == SUPPORTED_WALLETS.METAMASK">
          ...
          >
            ...
            <div class="waitTitle">Switch to BSC Wallet</div>
        ...
        <template v-if="sourceWalletType == SUPPORTED_WALLETS.BINANCE_CHAIN">
          <div class="walletIcon">
            <img
              ...
              src="@/static/transferProgress/bsc_wallet.svg"
            />
            <img class="arrow" src="@/static/transferProgress/long_arrow.svg" />
            <img
              class="wallteLogo"
              src="@/static/transferProgress/metamask_wallet.svg"
            />
        ...
        <div v-for="(item, index) in setupArray" :key="index" class="item">
          <div class="imgBox">
            <transition-group name="img-fade">
              ...
              <template v-else-if="currentStep == index && !currentErrMsg">
                <img
                  key="2"
                  class="loading"
          ...
          <div class="text" :class="{ current: currentStep == index }">
            {{ item }}
          </div>
          ...
          <template v-if="!isMobile">
            ...
            <div
              class="view"
              v-if="currentStep == index && currentConfirm && !currentErrMsg"
              @click.stop="openBlockchainBrowser(currentHash, currentNetworkId)"
            >
...
export default {
  ...
  methods: {
    ...
    jumpToSetup() {
      let url = "https://docs.binance.org/smart-chain/wallet/metamask.html";
